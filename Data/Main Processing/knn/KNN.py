...
neighbors = list(np.arange(2,16+1))
...
for i in range(len(neighbors)):
  t = threading.Thread(target = knn_model,
                       args = [neighbors[i],X_clean,y_clean])
...
knn_model(n,X_mean,y_mean)
nans_handled.append('Replaced with Mean')
knn_model(n,X_mode,y_mode)
nans_handled.append('Replaced with Mode')
knn_model(n,X_median,y_median)
